---
name: seo-aeo-audit
description: Runs a full SEO/AEO (search + AI answer-engine) audit for a website using connected Google Search Console and GA4 data, plus an on-page technical scan (titles, meta descriptions, H1s, content depth), then acts as an industry-fluent content expert to produce a content development plan — keyword targets, publishing timeline, and full publish-ready drafts — plus a prioritized fix-it queue and a branded HTML dashboard. Tracks recommendations across refresh runs so it reports whether a past fix worked instead of re-listing the same gap. Use whenever the user asks for an SEO audit, an on-page/technical SEO audit, an AEO/AI-search-visibility audit, a content strategy, or wants recommendations based on GSC/GA4/rankings/page titles/meta tags for any site — even without saying "audit." Also use to refresh a prior audit or run this for a different client/site.
---

# SEO / AEO Audit Skill

Produces a data-grounded SEO and AEO (answer-engine optimization) audit for one site at a time: what's actually happening in Search Console, GA4, and on the pages themselves (titles, meta descriptions, headings, content depth) — packaged as a fix-it queue, a content plan with full drafts, and a shareable HTML dashboard, in the spirit of tools like Semrush/Ubersuggest site audits but grounded in this site's real connected data. On repeat runs, it compares against the last audit so recommendations evolve instead of repeating.

Never invent metrics. Every number in the output must come from an actual tool call or page read. If a data source isn't available or returns nothing, say so plainly in the audit rather than estimating.
...
If a history file is found, load it and use the **most recent entry in its `audits` array** as the baseline to compare against. It contains, per prior recommendation: what was recommended, when, the baseline metric(s) for its target keyword/page at that time, and (if known) when the fix was implemented.
...
- Focus: general SEO health, on-page/technical health, local SEO ("near me" / service-area), AEO/AI-answer visibility, or all of the above
...
## 7. On-page technical scan (title tags, meta descriptions, H1s, content depth)

For every real page found in step 6, evaluate on-page fundamentals the way a Semrush/Ubersuggest site audit would — but only report what you can actually verify, per source:

- **Title tag**: presence, length (roughly 50-60 characters is the safe range), whether it includes the page's primary target keyword. **Caveat**: a chat-based `web_fetch` often strips `<head>` content via markdown extraction, so the title tag and meta description may not be visible that way. If running in Claude Code (or anywhere with direct file/HTML access), read the raw source instead — that's the reliable path. If only web-based fetch is available and `<head>` isn't visible in the result, say so explicitly rather than guessing pass/fail.
- **Meta description**: presence, length (roughly 120-155 characters), same access caveat as above.
- **H1**: exactly one per page, contains a relevant keyword, meaningfully distinct from the title tag (not an identical duplicate).
- **Heading hierarchy**: logical nesting (H2s nested under the H1, no skipped levels, no multiple H1s).
- **Content depth**: approximate word count of the visible body content. Don't compare against a fixed universal number ("pages need 1500 words") — instead spot-check 2-3 actual top-ranking pages for the same target query (`web_search` the query, `web_fetch` a couple of results) and compare relative depth. Report what you actually found on both sides.
- **Internal linking**: whether the page links to/from other relevant pages on the same site.
- **Image alt text**: spot-check if determinable from the fetched content; note if not verifiable through the current access method.
- **Page performance**: if the connected marketing tool also exposes a source like PageSpeed Insights or Core Web Vitals, pull it for a real performance score. Don't estimate load speed without a real source — omit this check entirely rather than guess.

Present this per-page as a pass/fail/unverifiable checklist, not a vague score — each item should be traceable to what was actually checked and how.

## 8. Compare against history (if a prior audit was found in step 1)
...
- **Unchanged / stalled** — implemented long enough ago (2+ weeks, ideally longer for content plays) with no meaningful movement in its target keyword. Don't just re-list the same recommendation again. Instead, diagnose *why* it might have stalled and propose a different next step, e.g.:
  ...
  - Indexed but thin/short relative to competitors, or failing the on-page checks from step 7 (missing title tag, no H1, etc.) — a very concrete, checkable cause now that step 7 exists
...
1. **Progress since last audit** (step 8 output, if applicable) — what worked, what stalled and why, before anything else
2. **Structural/technical** (indexing, sitemap, tracking installation) — usually highest-leverage, lowest-effort fixes
3. **On-page health** (step 7 output — missing/duplicate titles, missing meta descriptions, missing or multiple H1s, thin content) — concrete and highly actionable
4. **Local Pack / map cannibalization** (if applicable — local business with #1-position, 0-click queries)
5. **Content gaps** (non-branded queries with impressions but no dedicated page/content targeting them)
6. **AEO readiness** (see below)
...
## 10. Research the client's industry (become the expert)
...
## 11. Build the content plan: keyword targets + timeline

For each content gap identified in step 9 (including any stalled item from step 8 that needs a *different* approach, not a repeat), produce a plan entry with:
...
- **Why it closes an SEO or AEO gap** — be specific about which (e.g. "closes non-branded discovery gap" vs. "improves AI-answer citability" vs. "fixes an on-page health failure from step 7")
- **Timeline**: sequence items across a realistic publishing cadence (e.g. weekly or biweekly for a small business, not 10 pieces in week one) — order by effort-to-impact, with structural/on-page quick fixes from steps 7 and 9 done first, then this content rolled out afterward. State actual target dates or week numbers ("Week 1", "Week 3") relative to the audit date, not vague "soon."

Present this as a calendar/table (in the dashboard or as a companion file — see step 13) before writing full drafts, and give the user a chance to react to scope/sequencing if the conversation allows for a check-in, especially if the plan is large (6+ pieces).

## 12. Write full content drafts
...
- Include a suggested title tag (under ~60 chars) and meta description (under ~155 chars) — matching the exact length guidance used in step 7's checks
...
- Match the site's actual existing tone/voice from step 6, and reflect the industry fluency from step 10
...
Copy `assets/dashboard-template.html` to the workspace, then fill in every `{{PLACEHOLDER}}` with real figures and findings from steps 4-9, plus the content calendar from step 11 (each plan card should show its keyword targets and timeline slot, and link/name the corresponding draft file from step 12). If step 8 produced a progress comparison, give it its own section near the top of the dashboard, not folded silently into general findings. The on-page technical scan from step 7 gets its own section with a per-page pass/fail/unverifiable table — this is the section that makes the dashboard feel like a Semrush/Ubersuggest-style site audit rather than just a rankings report. Do not leave placeholder text in the final file — every section should reflect this site's actual data. Follow the file's inline comments for what belongs in each section. Save the final file as `<site-slug>-seo-audit.html`, present it alongside all content-draft files from step 12 via `present_files` (or the file-share method available in the current environment).

Keep the same information architecture as the template: stat strip → progress-since-last-audit (if applicable) → findings cards → on-page technical scan table → #1-rank/zero-click table (only include if applicable) → priority ticket queue (Now/Next/Later) → content plan cards per page (with keyword targets, timeline, draft filename) → AEO checklist → GA4 section (clearly labeled if data is early/thin/missing).

## 14. Save the history snapshot

Write/overwrite `history/<site-slug>-history.json` with this audit's state, so the *next* run (step 1) can compare against it, and so it can be pasted into the companion Audit Portfolio dashboard artifact for cross-client browsing. Use this exact schema so both consumers can parse it reliably:

```json
{
  "site_slug": "25thstreetbarbers",
  "site_name": "Barbers on 25th",
  "audits": [
    {
      "date": "2026-07-28",
      "items": [
        {
          "id": "murray-hill-page",
          "title": "Murray Hill neighborhood page",
          "type": "technical | on-page | content | aeo",
          "date_recommended": "2026-07-28",
          "date_implemented": "2026-08-01",
          "target_keyword": "murray hill barbershop",
          "baseline": {"position": 12, "impressions": 13, "clicks": 0},
          "current": {"position": 12, "impressions": 13, "clicks": 0},
          "status": "open | too_early | improved | stalled"
        }
      ]
    }
  ]
...
Append a new object to the `audits` array each run rather than overwriting prior entries — the full history of snapshots is what lets both this skill's step 8 comparison *and* the portfolio dashboard's trend view work. Reuse the same `id` for the same tracked item across audits so its trend can be followed over time; give new items introduced in this audit a new stable `id`.

Keep this file machine-readable and compact — it's read by this skill (and optionally pasted into the portfolio dashboard), not meant as a human-facing deliverable on its own. If the environment has no persistent storage (a one-off claude.ai conversation), still write it to the workspace and mention to the user that saving/re-uploading it next time is what enables the comparison in step 8.
...
- A recurring refresh (e.g. every 2-4 weeks) works well as a scheduled task in Claude Cowork, pointed at the same connectors and project folder — this is also what makes the history comparison in step 8 fully automatic, since Cowork's project folder persists between runs
- One-off technical fixes (schema, sitemap files, title/meta tag edits, code-level changes) are often faster to execute directly in Claude Code if the user has repo/file access — and give a more reliable on-page read than chat-based web_fetch, since Code can open the raw HTML source directly
- If managing multiple clients, the `history/<site-slug>-history.json` file from each can be pasted into the companion Audit Portfolio dashboard (a separate Claude artifact) to browse all clients' progress over time in one place, without needing live API/connector access from that dashboard
